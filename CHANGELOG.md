...
### Added
- **일반 대시보드 분포 차트에 커스텀 스마트 필터 차원 추가** — 분포 차트(파이/막대)의 분류 기준 셀렉트에 내장 차원(상태·담당자·타입·우선순위·라벨·보고자)과 함께 커스텀 스마트 필터가 `<이름>별` 로 노출됩니다. 각 값의 JQL에 매칭되는 이슈 수를 세며, 한 이슈가 여러 값에 매칭되면 라벨처럼 각각 카운트되고, 어떤 값에도 안 걸리는 이슈는 `(해당 없음)` 버킷으로 묶여 전체가 빠짐없이 표시됩니다. 선택한 커스텀 차원은 기존처럼 localStorage 에 저장되고, 해당 필터가 설정에서 삭제되면 "상태별"로 자동 복귀합니다.
  - 순수 헬퍼 `countCustomFacetValues` 추가 (`lib/resolution-time`) — 컴파일 실패한 값은 건너뜁니다.
...
### Fixed
- **커스텀 스마트 필터에 새 커스텀 필드 추가 시 필터가 안 먹던 문제** — 서버 fetch는 스마트 필터·비율 분석이 참조하는 커스텀 필드를 매번 취합해 요청하지만, 클라이언트 이슈 캐시 키가 대시보드 id뿐이라 설정에서 필드를 추가해도 (staleTime 이내면) 그 필드 없이 받아둔 캐시를 재사용해 새 필터가 아무것도 매칭하지 못했습니다. 이제 참조 커스텀 필드 목록의 지문(`customFieldFingerprint`)을 쿼리 키에 포함해, 필드 구성이 바뀌면 대시보드 로딩 시 즉시 새 필드를 포함해 다시 불러옵니다.

[1.29.1]: https://github.com/k31001/jira-collector/releases/tag/v1.29.1
...
### Added
- **JQL별 시간축 오프셋** — 해결 시간 대시보드의 각 JQL에 "시간축 오프셋 (일)"을 설정할 수 있습니다(± 가능). 시작 시점이 다른 프로젝트끼리 같은 시간축에서 추이를 겹쳐 비교할 때 사용합니다. 오프셋은 시간축 차트(평균 해결 시간 추이 · 미해결 추이 · 처리량 · 비율 분석)와 마일스톤 수직선에만 적용되며, 해결 시간 값 자체를 쓰는 요약 카드·분포 히스토그램·슬로우/에이징 분석은 실제 날짜 그대로 유지됩니다. 오프셋이 걸린 시리즈는 범례 라벨에 `(+30일 이동)` 처럼 표시돼 이동된 시리즈임을 알 수 있습니다.
  - DB: `resolution_dashboard_sources.time_offset_days` 컬럼 추가 (기본 0, 마이그레이션 `0006`).
  - 순수 헬퍼 `shiftIssuesTime` / `shiftDateOnly` 추가 — created/resolved/updated 를 같은 양만큼 이동시켜 기간(duration)은 보존합니다.

[1.29.0]: https://github.com/k31001/jira-collector/releases/tag/v1.29.0
...
### Changed
- **해결 시간 분포 히스토그램의 꼬리 구간 자동 확장** — 고정 버킷(예: 24시간) 단위는 **1주까지만** 적용하고, 그 이후는 1주–2주 → 2주–1달 → 1달–3달 → 3달+ 로 구간이 점점 넓어집니다. 기존에는 12개 고정 폭 빈을 지나면 전부 하나의 overflow 빈(`288h+`)에 뭉쳐 긴 꼬리의 분포가 보이지 않았는데, 이제 오래 걸린 이슈도 주/월 단위로 나뉘어 보입니다. 버킷이 아주 작으면 고정 폭 빈은 최대 12개까지만 만들고 바로 확장 구간으로 이어지며, 버킷이 1주 이상이면 고정 폭 빈 하나 뒤에 남은 확장 구간만 붙습니다. 경계는 정확히 1주(168h)/2주(336h)/1달(720h)/3달(2160h)이고 라벨도 `1주–2주` 식으로 표시됩니다.

[1.28.0]: https://github.com/k31001/jira-collector/releases/tag/v1.28.0
...
### Added
- **사이드바 접기(공간 확보)** — 왼쪽 메뉴 헤더의 접기 버튼으로 사이드바를 숨겨 본문이 가로 공간을 더 쓰도록 했습니다. 접으면 너비 48px의 슬림 레일(열기 버튼 + 테마 토글)만 남아 언제든 다시 펼 수 있고, `⌘B`/`Ctrl+B` 단축키로도 토글됩니다. 접힘 상태는 `sidebar-collapsed` 로 localStorage 에 저장되며, 서버는 펼친 상태로 렌더한 뒤 마운트 후 적용해 hydration mismatch 를 피합니다.

[1.27.0]: https://github.com/k31001/jira-collector/releases/tag/v1.27.0
...
### Added
- **스마트 필터 기반 분포 차트(파이/막대)** — 일반 대시보드에 스마트 필터 결과를 시각화하는 접이식 "분포 차트" 섹션을 추가했습니다. 상태·담당자·타입·우선순위·라벨·보고자 중 하나를 분류 기준으로 골라 **파이/막대**를 전환하며 볼 수 있습니다.
  - 차트 집계는 스마트 필터 드롭다운과 동일한 `buildFacets` 를 **필터링된 이슈 집합** 위에서 계산하므로, 차트 합계가 항상 필터된 표/상태 바와 일치합니다(예: 타입=Bug 선택 시 "필터된 97개").
  - 고유값이 많은 차원(담당자·라벨)은 상위 11개만 표시하고 나머지는 "기타"로 묶어 가독성을 유지합니다. 파이 슬라이스/막대는 동일 팔레트로 색을 공유합니다.
  - 열림 여부·분류 기준·차트 종류는 대시보드별 `facet-charts:{id}` 로 localStorage 영속화(트렌드 차트와 동일한 SSR 안전 로딩 방식).

[1.25.0]: https://github.com/k31001/jira-collector/releases/tag/v1.25.0
